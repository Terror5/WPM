...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `Project`.`User_Role` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `Project`.`OpenUpRole` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `Project`.`User_Project_Role` ;
...
-- -----------------------------------------------------
-- Table `Project`.`TaskLog`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `Project`.`TaskLog` ;

CREATE TABLE IF NOT EXISTS `Project`.`TaskLog` (
  `idTask` INT UNSIGNED NOT NULL,
  `dayIteration` DATE NOT NULL,
  `hoursRemaining` DECIMAL(3,2) NULL,
  INDEX `fk_TaskLog_Task1_idx` (`idTask` ASC),
  PRIMARY KEY (`idTask`, `dayIteration`),
  CONSTRAINT `fk_TaskLog_Task1`
    FOREIGN KEY (`idTask`)
    REFERENCES `Project`.`Task` (`idTask`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
INSERT INTO `project`.`openuprole`
(`idOpenUpRole`,
`title`)
VALUES
('PM','Projektmanager'),
